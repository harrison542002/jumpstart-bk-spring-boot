package com.jumpstart.org.utils;
...
public class JwtUtils {
	...
	public String createToken(Authentication authentication) {
		...
		expiryDate.setTime(date.getTime() + Long.parseLong(appProperties.getAuth().getTokenExpirationMsec()));
	...
	public String createTokenWhileRegistration(SignUpRequest signUpRequest){
		Date date = new Date();
		Date expiryDate = new Date();
		expiryDate.setTime(date.getTime() + Long.parseLong(appProperties.getAuth().getTokenExpirationMsec()));
		return Jwts.builder().setSubject(signUpRequest.getEmail()).setIssuedAt(date).setExpiration(expiryDate)
				.signWith(SignatureAlgorithm.HS512, appProperties.getAuth().getTokenSecret()).compact();
	}
